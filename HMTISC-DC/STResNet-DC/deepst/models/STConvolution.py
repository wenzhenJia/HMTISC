...
def seqCNN(n_flow=4, seq_len=3, map_height=32, map_width=32):
    model = Sequential()
    model.add(Conv2D(64, 3, input_shape=(n_flow*seq_len, map_height, map_width),padding='same'))
    ...
    model.add(Conv2D(n_flow, 3,  padding='same'))
...
def seqCNN_CPT(c_conf=(4, 3, 32, 32), p_conf=(4, 3, 32, 32), t_conf=(4, 3, 32, 32)):
    ...
    for conf in [c_conf, p_conf, t_conf]:
        if conf is not None:
            components.append(seqCNNBaseLayer1(conf).output)
            nb_flow = conf[0]\
 # model.add(Merge(components, mode='concat', concat_axis=1))  # concat
    model.add(merge[components,seqCNNBaseLayer1(conf).output], mode='concat', concat_axis=1)###################################
    model.add(Conv2D(64, 3,padding='same'))
    model.add(Activation('relu'))
    ...
    model.add(Conv2D(nb_flow, 3,padding='same'))
    model.add(Activation('tanh'))
    return model
...
def seqCNN_CPT2(c_conf=(4, 3, 32, 32), p_conf=(4, 3, 32, 32), t_conf=(4, 3, 32, 32)):
    ...
    for conf in [c_conf, p_conf, t_conf]:
        if conf is not None:
            components.append(seqCNNBaseLayer1_2(conf).output)
            nb_flow = conf[0]
    # model.add(Merge(components, mode='concat', concat_axis=1))  # concat
    if len(components) > 1:
        # model.add(Merge(components, mode='sum'))
        model.add(merge[components,seqCNNBaseLayer1_2(conf).output], mode='sum')##########################################################
    else:
        model = components[0]
    model.add(Activation('relu'))
    model.add(Conv2D(64, 3,padding='same'))
...
def seqCNN_CPTM(c_conf=(4, 3, 32, 32), p_conf=(4, 3, 32, 32), t_conf=(4, 3, 32, 32), metadata_dim=None):
    ...
    model_final=merge([model.output,metadata_processor.output])###############################
...
def lateFusion(metadata_dim, n_flow=2, seq_len=3, map_height=32, map_width=32):
    ...
    model=merge([mat_model.output,metadata_processor.output])#############################
    model.add(Activation('tanh'))
    return model
...
def seq3DCNN(n_flow=4, seq_len=3, map_height=32, map_width=32):
    ...
    model.add(Conv3D(64, 2, 3,border_mode='same', padding=(n_flow, seq_len, map_height, map_width)))
    model.add(Activation('relu'))

    model.add(Conv3D(128, 2, 3, padding='same'))
    model.add(Activation('relu'))

    model.add(Conv3D(64, 2, 3,padding='same'))
    model.add(Activation('relu'))

    model.add(ZeroPadding3D(padding=(0, 1, 1)))
    model.add(Conv3D(n_flow, seq_len, 3,padding='valid'))
